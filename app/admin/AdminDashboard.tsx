...
function VoteAdjustControl({ contestantId, category, currentAdjustment, onSaved }: {
  contestantId: string
  category: string
  ...
  if (!editing) {
    return (
      <button
        ...
        className="text-xs text-zinc-500 hover:text-yellow-400 border border-white/10 hover:border-yellow-700/40 px-2 py-1 rounded-lg transition-colors shrink-0"
  ...
  return (
    <div className="flex items-center gap-1 shrink-0">
      <button onClick={() => setValue(v => String(parseInt(v || '0') - 1))}
        className="w-6 h-6 rounded bg-red-900/40 text-red-400 hover:bg-red-900/60 text-sm font-bold flex items-center justify-center">−</button>
      <input
        type="number"
        value={value}
        onChange={e => setValue(e.target.value)}
        className="w-14 bg-black/40 border border-yellow-700/40 text-white text-xs text-center rounded px-1 py-1 focus:outline-none"
        onKeyDown={e => { if (e.key === 'Enter') save(); if (e.key === 'Escape') setEditing(false) }}
        autoFocus
      />
      <button onClick={() => setValue(v => String(parseInt(v || '0') + 1))}
        className="w-6 h-6 rounded bg-green-900/40 text-green-400 hover:bg-green-900/60 text-sm font-bold flex items-center justify-center">+</button>
      <button onClick={save} disabled={saving}
        className="text-xs bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-2 py-1 rounded disabled:opacity-50">
        {saving ? '…' : '✓'}
      </button>
              ...
              <div key={cat} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-zinc-500 text-xs mt-0.5">{catTotal} vote{catTotal !== 1 ? 's' : ''} (displayed)</div>
                  </div>
                      ...
                      return (
                        <div key={r.contestantId} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-center mb-1">
                              ...
                              <div className="flex items-center gap-1.5 ml-2 shrink-0">
                                {(r.adjustment ?? 0) !== 0 && (
                                  <span className="text-zinc-500 text-xs line-through">{r.count}</span>
                                )}
                                <span className="text-yellow-400 text-xs font-bold">{display} ({pct}%)</span>
                                {(r.adjustment ?? 0) !== 0 && (
                                  <span className={`text-xs font-semibold ${(r.adjustment ?? 0) > 0 ? 'text-green-400' : 'text-red-400'}`}>
                                    {(r.adjustment ?? 0) > 0 ? `+${r.adjustment}` : r.adjustment}
                                  </span>
                                )}
                              </div>
                            </div>
                          ...
                          <VoteAdjustControl
                            ...
                            category={cat}
                            currentAdjustment={r.adjustment ?? 0}
                            onSaved={fetchVotes}
                          />
                        </div>
                      )
