...
// Rate limit: simple in-memory map (resets on cold start — acceptable for free tier)
...
export async function POST(req: NextRequest) {
  ...
  const { application_id, mobile } = parsed.data
  ...
  return new Response(
    JSON.stringify({ success: true, application_id: reg.application_id, name: reg.full_name }),
    { status: 200, headers }
  )
}
